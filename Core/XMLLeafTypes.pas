...
type
  ...
  // 可扩展的基本类型枚举变量
  XMLTypes = (xs_ENTITIES, xs_ENTITY, xs_ID, xs_IDREF, xs_IDREFSlanguage,
    xs_Name, xs_NCName, xs_NMTOKEN, xs_NMTOKENS, xs_normalizedString, xs_QName,
    xs_string, xs_token, xs_date, xs_time, xs_dateTime, xs_duration, xs_byte,
    xs_decimal, xs_int, xs_integer, xs_long, xs_negativeInteger,
    xs_nonNegativeInteger, xs_nonPositiveInteger, xs_positiveInteger, xs_short,
    xs_unsignedLong, xs_unsignedInt, xs_unsignedShort, xs_unsignedByte,
    xs_anyURI, xs_base64Binary, xs_boolean, xs_double, xs_float, xs_hexBinary,
    xs_NOTATION, xs_Class, xml_XMLType, xml_Complex, xml_Coordinate,
    xml_ArrayCoordinates, xml_Double1D, xml_Integer1D, xml_String1D,
    xml_Double2D, xml_Double3D, xml_Integer2D, xml_Integer3D, xml_Byte1D, xml_Boolean1D,
    xml_Complex1D, xml_Byte2D, xml_Boolean2D, W_Point2I, W_Point2D, W_Point3I, W_Point3D,
    W_Point2Is, W_Point2Ds, W_Point3Is, W_Point3Ds, xml_Color, xml_Pointer);
...
const
  XMLTypeStrings: array [XMLTypes] of string = ('xs_ENTITIES', 'xs_ENTITY',
    'xs_ID', 'xs_IDREF', 'xs_IDREFSlanguage', 'xs_Name', 'xs_NCName',
    'xs_NMTOKEN', 'xs_NMTOKENS', 'xs_normalizedString', 'xs_QName', 'xs_string',
    'xs_token', 'xs_date', 'xs_time', 'xs_dateTime', 'xs_duration', 'xs_byte',
    'xs_decimal', 'xs_int', 'xs_integer', 'xs_long', 'xs_negativeInteger',
    'xs_nonNegativeInteger', 'xs_nonPositiveInteger', 'xs_positiveInteger',
    'xs_short', 'xs_unsignedLong', 'xs_unsignedInt', 'xs_unsignedShort',
    'xs_unsignedByte', 'xs_anyURI', 'xs_base64Binary', 'xs_boolean',
    'xs_double', 'xs_float', 'xs_hexBinary', 'xs_NOTATION', 'xs_Class',
    'xml_XMLType', 'xml_Complex', 'xml_Complex1D', 'xml_Coordinate',
    'xml_ArrayCoordinates', 'xml_Double1D', 'xml_Integer1D', 'xml_String1D',
    'xml_Double2D', 'xml_Double3D', 'xml_Integer2D', 'xml_Integer3D', 'xml_Byte1D', 'xml_Boolean1D',
    'xml_Byte2D', 'xml_Boolean2D', 'W_Point2I', 'W_Point2D', 'W_Point3I',
    'W_Point3D', 'W_Point2Is', 'W_Point2Ds', 'W_Point3Is', 'W_Point3Ds',
    'xml_Color', 'xml_Pointer');
  XMLColors: array [0 .. 19] of TAlphaColor = (TAlphaColor($FFFF0000),
    TAlphaColor($FFFF7700), TAlphaColor($FFFFFF00), TAlphaColor($FF77FF00),
    TAlphaColor($FF00FF00), TAlphaColor($FF00FF77), TAlphaColor($FF00FFFF),
    TAlphaColor($FF0077FF), TAlphaColor($FF0000FF), TAlphaColor($FF7700FF),
    TAlphaColor($FFFF00FF), TAlphaColor($FFFF0077), TAlphaColor($FF000000),
    TAlphaColor($FF90EE90), TAlphaColor($FF828282), TAlphaColor($FFA020F0),
    TAlphaColor($FF8B8970), TAlphaColor($FF20B2AA), TAlphaColor($FF87CEFA),
    TAlphaColor($FFA52A2A));
...
begin
  case tp of
    ...
    xs_decimal:
      Result := name + '.ToDouble';
    xs_int:
      Result := name + '.ToInteger';
    xs_integer:
      Result := name + '.ToInteger';
    xs_long:
      Result := name + '.ToInt64';
    ...
    xs_double:
      Result := name + '.ToDouble';
    xs_float:
      Result := name + '.ToSingle';
    xs_hexBinary:
      Result := 'String2Hex(' + name + ')';
    xs_NOTATION:
      Result := 'StrToxs_NOTATION';
    xs_Class:
      Result := 'StrToxs_Class';
    xml_Complex:
...
begin
  ...
  for I := 0 to count - 1 do
  begin
    Result[I] := TComplex.CreateFromString(strs[I]);
  end;
  SetLength(strs, 0);
end;
...
begin
  ...
  for I := 0 to count - 1 do
  begin
    Result[I] := String2Boolean(strs[I]);
  end;
  SetLength(strs, 0);
end;
...
var
  ...
  strs: TArray<string>;
begin
  strs := value.Split([#13, #10], TStringSplitOptions.ExcludeEmpty);
  count := Length(strs);
  SetLength(Result, count);
  for I := 0 to count - 1 do
  begin
    Result[I] := String2Integer1D(strs[I]);
  end;
  SetLength(strs, 0);
end;
...
begin
  ...
  for I := 0 to count - 1 do
  begin
    Result[I] := String2Integer2D(strs[I]);
  end;
  SetLength(strs, 0);
end;
...
begin
  ...
  for I := 0 to count - 1 do
  begin
    Result[I] := String2Double1D(strs[I]);
  end;
  SetLength(strs, 0);
end;
...
begin
  ...
  for I := 0 to count - 1 do
  begin
    Result[I] := String2Double2D(strs[I]);
  end;
  SetLength(strs, 0);
end;
...
begin
  ...
  for I := 0 to count - 1 do
  begin
    Result[I] := String2Boolean1D(strs[I]);
  end;
  SetLength(strs, 0);
end;
...
begin
  Result := FloatToStr(Longitude) + ',' + FloatToStr(Latitude) + ',' +
    FloatToStr(Altitude);
end;
